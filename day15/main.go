...
func canBoxMove(house *Warehouse, box Pair, dir rune) bool {
	...
	if ok {
		return canBoxMove(house, nextCoords, dir) // can I move box in front
	}
...
func main() {
	...
	house := readData(fileName)
	house2 := readData(fileName)
	fmt.Println(solveP1(house))
	bigHouse := NewBigHouse(house2)
	fmt.Println(solveP2(bigHouse))
}
